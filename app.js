...
// 6. Brújula: rotar mapa según orientación del dispositivo
var brujulaActiva = false;
var brujulaListener = null;
    ...
    if (typeof DeviceOrientationEvent !== 'undefined' &&
        ...
        DeviceOrientationEvent.requestPermission().then(function(state) {
            if (state === 'granted') {
                ...
                mostrarToast('Permiso de brújula denegado');
            }
        }).catch(function() {
            mostrarToast('No se pudo solicitar permiso de brújula');
        });
    } else if ('DeviceOrientationEvent' in window) {
        iniciarBrujula();
    } else {
        mostrarToast('Tu dispositivo no soporta brújula');
    }
}
...
function iniciarBrujula() {
    ...
    if (btn) {
        ...
        btn.style.color = '#fff';
        btn.title = 'Desactivar brújula';
    }
    mostrarToast('Brújula activada');

    brujulaListener = function(e) {
        var heading = null;
    ...
    window.addEventListener('deviceorientationabsolute', brujulaListener, true);
    window.addEventListener('deviceorientation', brujulaListener, true);
}
...
function desactivarBrujula() {
    ...
    if (brujulaListener) {
        window.removeEventListener('deviceorientationabsolute', brujulaListener, true);
        window.removeEventListener('deviceorientation', brujulaListener, true);
        brujulaListener = null;
    }
